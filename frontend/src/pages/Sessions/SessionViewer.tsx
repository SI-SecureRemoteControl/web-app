    ...
    const exportLogs = async (type: 'txt' | 'csv' | 'xlsx') => {
        ...
        if (type === 'xlsx') {
            ...
            sessionLogs.forEach(session => {
                ...
                const endEvent = session.events.find(event => event.type === 'session_end' || event.type === 'inactive_disconnect' || event.type === 'session_expired' || event.type === 'terminate_session');
